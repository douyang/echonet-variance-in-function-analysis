    ...
    Args:
        ...
        target_type (string or list, optional): Type of target to use,
            ...
            The targets represent:
                ...
                ``NormalizedLargeIndex'' (int): normalized index of large (diastolic) frame in video
                ``NormalizedSmallIndex'' (int): normalized index of small (systolic) frame in video
    ...
    def __getitem__(self, index):
        ...
        for t in self.target_type:
            ...
            if t == "Filename":
                ...
                rangeOfClips = np.arange(large_index - (length - 1) * self.period, large_index)
                if large_index > 0 and large_index <= length:
                    self.selectiveSample = False
                else:
                    large_index = np.random.choice(rangeOfClips, self.clips)[0] - rangeOfClips[0]
                    self.selectiveSample = True
            elif t == "NormalizedSmallIndex":
                # Normalized small (systolic) frame is first
                small_index = np.int(self.frames[key][0])
                rangeOfClips = np.arange(small_index - (length - 1) * self.period, small_index)
                if small_index > 0 and small_index <= length:
                    self.selectiveSample = False
                else:
                    small_index = np.random.choice(rangeOfClips, self.clips)[0] - rangeOfClips[0]
                    self.selectiveSample = True
                
                target.append(np.float32(small_index))
            elif t == "LargeFrame":
