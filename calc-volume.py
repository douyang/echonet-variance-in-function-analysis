...
for i in range(len(df)):
  ...
  if os.path.exists(frameCapturePath):
    functions.getSpecificFrameAndCrop(dataPath, frameCapturePath, int(frame))
...
for i in range(len(paths)):
  ...
  if os.path.exists(pathName):
    ...
    volume, x1, y1, x2, y2 = functions.calculateVolume(pathName, numberValue)
